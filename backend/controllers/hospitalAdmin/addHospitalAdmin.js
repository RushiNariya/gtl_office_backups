...
const addHospitalAdmin = async (req, res) => {
    ...
    try {
        if (validateHospitalAdminBody(req.body)) {
            ...
            const userQuery = `insert into users (firstname, lastname, email, phone, birthdate, address_id, password, role_id, is_active) values('${firstname}', '${lastname}', '${email}','${phone}', '${birthdate}','${addressResult.rows[0].id}','${bcryptPassword}',2, true) RETURNING id,concat(firstname, ' ', lastname) as name, email`;
