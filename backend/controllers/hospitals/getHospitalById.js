...
const getHospitalById = async (req, res) => {
    try {
        ...
        const hospitalQuery = `select h.id, h.name, h.email, h.contact_no, h.diseases, h.hours_of_operation, h.website, a.city, a.pincode, a.state_id, a.street, s.name as state_name from hospitals h join address a on h.address_id = a.id join states s on a.state_id = s.id where h.id = '${hospitalId}' and h.is_active = true`;
