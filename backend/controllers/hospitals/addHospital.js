...
const addHospital = async (req, res) => {
    ...
    try {
        if (validateHospitalBody(req.body)) {
            ...
            const HospitalQuery = `insert into hospitals (name, email, contact_no, hours_of_operation, diseases, website, address_id) values('${name}', '${email}', '${contactNo}','${hoursOfOperation}', '${diseases}','${website}','${addressResult.rows[0].id}') RETURNING id, name`;
